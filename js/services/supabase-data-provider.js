(function supabaseProviderFactory(global){
  ...
  async function request(path,options={}){const token=options.auth?global.LEFUSIL_AUTH?.getAccessToken():key;if(options.auth&&!token)throw new errors.ServiceError('AUTH_REQUIRED','An authenticated staff session is required.');const response=await fetch(`${base}${path}`,{...options,headers:{apikey:key,Authorization:`Bearer ${token}`,'Content-Type':'application/json',Accept:'application/json',...(options.headers||{})}});if(!response.ok){const payload=await response.json().catch(()=>({})),reference=response.headers.get('x-request-id')||`HTTP-${response.status}`;throw new errors.ServiceError(response.status===401||response.status===403?'ACCESS_DENIED':'SERVICE_UNAVAILABLE',payload.message||payload.hint||'The requested service is unavailable.',{reference})}return response.status===204?null:response.json()}
  const adminOnly=method=>async()=>{throw new errors.ServiceError('NOT_IMPLEMENTED',`${method} is not connected in this stage.`)};
  const adminSelect='*,brand:brands(id,name,slug),category:categories(id,name,slug),product_images(id,storage_bucket,storage_path,alt_text,image_role,display_order,is_active)';
  const provider={
