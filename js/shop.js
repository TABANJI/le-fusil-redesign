(function collectionPage(){
  ...
  const products=rawProducts.map((item,index)=>({...item,_index:index,id:item.id??`product-${index}`,slug:clean(item.slug)||`product-${index}`,name:clean(item.name)||'Product details on request',brand:clean(item.brand)||'LE FUSIL',category:clean(item.category)||'Uncategorised',price:Number(item.price)>0?Number(item.price):null,priceOnRequest:Boolean(item.priceOnRequest)||!(Number(item.price)>0),availabilityKey:availabilityOf(item.availability),searchable:[item.name,item.brand,item.category,item.sku,item.shortDescription,...(Array.isArray(item.tags)?item.tags:[])].map(clean).join(' ').toLowerCase()}));
  ...
  const urlKeys=['category','brand','availability','price','condition'];
  const params=new URLSearchParams(location.search);
  const fromParam=key=>params.get(key)?.split(',').map(clean).filter(Boolean)||[];
  let state={category:fromParam('category'),brand:fromParam('brand'),availability:fromParam('availability'),price:fromParam('price'),condition:fromParam('condition'),origin:fromParam('origin'),featured:params.get('featured')==='true',q:clean(params.get('q')),sort:params.get('sort')||'featured'};
  let draft={...state};
  ...
  function openDrawer(){draft={...state};syncForm(draft);const panel=document.querySelector('#filters');drawerReturnFocus=document.activeElement;panel.classList.add('open');panel.setAttribute('aria-hidden','false');document.querySelector('#openFilters').setAttribute('aria-expanded','true');document.body.classList.add('lock');document.querySelector('#closeFilters').focus();document.querySelector('#draftCount').textContent=filtered(draft).length}
  ...
  document.querySelector('#drawerClear').addEventListener('click',()=>{draft={category:[],brand:[],availability:[],price:[],condition:[],origin:[],featured:false,q:state.q,sort:state.sort};syncForm(draft);document.querySelector('#draftCount').textContent=filtered(draft).length});
  ...
  addEventListener('popstate',()=>{const current=new URLSearchParams(location.search);state={category:current.get('category')?.split(',').map(clean).filter(Boolean)||[],brand:current.get('brand')?.split(',').map(clean).filter(Boolean)||[],availability:current.get('availability')?.split(',').map(clean).filter(Boolean)||[],price:current.get('price')?.split(',').map(clean).filter(Boolean)||[],condition:current.get('condition')?.split(',').map(clean).filter(Boolean)||[],origin:current.get('origin')?.split(',').map(clean).filter(Boolean)||[],featured:current.get('featured')==='true',q:clean(current.get('q')),sort:current.get('sort')||'featured'};search.value=state.q;document.querySelector('#searchClear').hidden=!state.q;sort.value=state.sort;syncForm();render()});
