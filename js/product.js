(function productPage(){
  ...
  const featureFallback=[
    {title:'Curated Selection',text:`Selected within the ${product.category||'sporting'} collection for personal boutique presentation.`,icon:'verified'},
    {title:'Personal Guidance',text:'Complete details and suitability are discussed directly with the LE FUSIL team.',icon:'guidance'},
    {title:'In-Store Verification',text:'Availability and applicable requirements are confirmed through the boutique.',icon:'compliance'}
  ];
  const features=Array.isArray(product.features)&&product.features.length?product.features.map((item,index)=>typeof item==='string'?{title:item,text:'Contact the boutique for complete details.',icon:['verified','guidance','compliance'][index%3]}:item):featureFallback;
  ...
  mount.innerHTML=`
    ...
    <section class="product-layout" aria-labelledby="productTitle">
      ...
      <aside class="product-info">
        <div class="info-intro"><div class="product-brand">${escapeHtml(product.brand||'LE FUSIL')}</div><h1 class="product-title" id="productTitle">${escapeHtml(product.name)}</h1>${clean(product.category)?`<p class="product-category">${escapeHtml(product.category)}</p>`:''}<div class="product-status-row"><span class="availability ${status.className}">${status.label}</span><span class="product-reference">Ref. ${escapeHtml(reference)}</span></div></div>
    ...
    <section class="product-section story-layout"><div class="story-mark"><div class="eyebrow">The Story</div></div><div class="story-content"><h2>A considered sporting selection.</h2><p>${escapeHtml(description)}</p>${product.description&&product.shortDescription&&product.description!==product.shortDescription?`<p>${escapeHtml(product.shortDescription)}</p>`:''}</div></section>
    <section class="product-section"><div class="section-heading"><div class="eyebrow">Product Character</div><h2>Key Features</h2></div><div class="feature-grid">${features.slice(0,4).map(item=>`<article class="feature-item"><div class="feature-icon">${icon(item.icon||'verified')}</div><h3>${escapeHtml(item.title||'Product Detail')}</h3><p>${escapeHtml(item.text||item.description||'Contact the boutique for complete details.')}</p></article>`).join('')}</div></section>
    ${specRows.length?`<section class="product-section"><div class="section-heading"><div class="eyebrow">Product Details</div><h2>Technical Specifications</h2></div><dl class="spec-list">${specRows.map(([label,value])=>`<div class="spec-row"><dt class="spec-label">${escapeHtml(label)}</dt><dd class="spec-value">${escapeHtml(value)}</dd></div>`).join('')}</dl></section>`:''}
