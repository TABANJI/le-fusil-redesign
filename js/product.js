(function productPage(){
  ...
  mount.innerHTML=`
    ...
    <section class="product-section story-layout"><div class="story-mark"><div class="eyebrow">About This Piece</div></div><div class="story-content"><h2>${escapeHtml(product.brand)} ${escapeHtml(product.name)}</h2><p>${escapeHtml(description)}</p></div></section>
    <section class="product-section"><div class="section-heading"><div class="eyebrow">Product Character</div><h2>Key Features</h2></div><div class="feature-grid">${features.slice(0,3).map(item=>`<article class="feature-item"><div class="feature-icon">${icon(item.icon||'verified')}</div><h3>${escapeHtml(item.title||'Product Detail')}</h3><p>${escapeHtml(item.text||item.description||'Contact the boutique for complete details.')}</p></article>`).join('')}</div></section>
